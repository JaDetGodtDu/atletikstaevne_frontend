...
export default function ResultTable() {
  ...
  const [formResult, setFormResult] = useState<Result>({
    ...
    contestant: { id: 0, name: "", club: "", age: new Date(), sex: "", disciplines: [] },
    date: new Date(),
  });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            onChange={(e) =>
              setFormResult({
                ...formResult,
                contestant: { ...formResult.contestant, id: parseInt(e.target.value) },
              })
            }
            required
          />
          <Select
            ...
            value={disciplines
              .filter((d) => d.id === formResult.discipline.id)
              .map((d) => ({ value: d.id, label: d.name }))}
            // onChange={(selectedOption) =>
            //   setFormResult({
            //     ...formResult,
            //     discipline: disciplines.find((d) =>
            //         d.id === selectedOption.value) || formResult.discipline,
            //   })
            // }
            onChange={(selectedOption) => {
              if (selectedOption) {
