...
import {
  ...
  getContestantById,
  handleContestant,
  deleteContestant as apiDeleteContestant,
} from "../services/api/contestantApi";
...
export default function ContestantTable() {
  ...
  const [formContestant, setFormContestant] = useState<Contestant>({
    id: 0,
    name: "",
    age: new Date(),
    club: "",
  ...
  useEffect(() => {
    ...
    if (filters.club) {
      result = result.filter((contestant) =>
        contestant.club.toLowerCase().includes(filters.club.toLowerCase())
      );
    }
    if (filters.discipline) {
      result = result.filter((contestant) =>
        contestant.disciplines.some(
          (d) => d.name.toLowerCase() === filters.discipline.toLowerCase()
        )
      );
    }
    if (filters.ageGroup) {
  ...
  const updateContestant = async (id: number, contestantToUpdate: Contestant) => {
    try {
      ...
      setContestants(
        contestants.map((contestant) => (contestant.id === id ? updatedContestant : contestant))
      );
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          <Select
            ...
            value={formContestant.disciplines.map((d) => ({
              value: d.id,
              label: d.name,
              resultType: d.resultType,
            }))}
          ...
          <tbody>
            {filteredContestants.map((contestant) => (
              <tr key={contestant.id}>
                ...
                <td>
                  {(() => {
                    ...
                    const hasHadBirthdayThisYear =
                      ...
                      (new Date().getMonth() === ageDate.getMonth() &&
                        new Date().getDate() >= ageDate.getDate());
                ...
                <td>{contestant.disciplines.map((discipline) => discipline.name).join(", ")}</td>
                <td>
